#!/usr/bin/env python3
"""Build the self-contained macOS Footnote.app and release ZIP."""
...
def build() -> Path:
    if sys.platform != "darwin":
        raise SystemExit("Footnote v0.1 app packaging currently supports macOS only")
    build_environment = os.environ.copy()
    build_environment["PYINSTALLER_CONFIG_DIR"] = str(BUILD / "cache")
    subprocess.run(
        [
            sys.executable, "-m", "PyInstaller", "--noconfirm", "--clean", "--onedir",
            "--name", "footnote-runtime", "--distpath", str(BUILD / "dist"),
            ...
            "--hidden-import", "uvicorn.lifespan.on", str(ROOT / "footnote_app.py"),
        ],
        cwd=ROOT,
        env=build_environment,
        check=True,
    )
    app = DIST / "Footnote.app"
    if app.exists():
        shutil.rmtree(app)
    ...
    shutil.copytree(BUILD / "dist" / "footnote-runtime", resources / "footnote-runtime")
    shutil.copy2(ROOT / "assets" / "Footnote.icns", resources / "Footnote.icns")
    launcher = macos / "Footnote"
    launcher.write_text('#!/bin/sh\nHERE="$(cd "$(dirname "$0")/../Resources/footnote-runtime" && pwd)"\nexec "$HERE/footnote-runtime" start\n', encoding="utf-8")
    launcher.chmod(0o755)
    with (app / "Contents" / "Info.plist").open("wb") as plist:
        plistlib.dump({
            "CFBundleDisplayName": "Footnote", "CFBundleExecutable": "Footnote",
            "CFBundleIdentifier": "com.footnote.memory", "CFBundleName": "Footnote",
            "CFBundleIconFile": "Footnote.icns",
            "CFBundlePackageType": "APPL", "CFBundleShortVersionString": VERSION,
            "CFBundleVersion": VERSION, "LSMinimumSystemVersion": "12.0",
        }, plist)
    verify_release_tree(app)
    archive_base = DIST / f"Footnote-macOS-arm64-{VERSION}"
    archive = Path(shutil.make_archive(str(archive_base), "zip", root_dir=DIST, base_dir="Footnote.app"))
    return archive
