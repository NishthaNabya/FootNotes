(function () {
  ...
  console.log(`[FootNotes] Memory cache initialized. Size: ${tweetCache.size}`);
  ...
  window.fetch = async function (input, init) {
    ...
    if (url.includes(BOOKMARK_OPERATION) || isBookmarkRequest(init)) {
      ...
      if (tweetId) {
        ...
        if (cachedTweet) {
          ...
          window.postMessage({ source: "footnotes-interceptor", type: "bookmark-captured", payload }, "*");
        }
      }
    }
    ...
    try {
      ...
      if (tweetsFound > 0) console.log(`[FootNotes] Cached ${tweetsFound} tweet(s)`);
    } catch (err) {}
    return response;
  };
  ...
  XMLHttpRequest.prototype.open = function (method, url) {
    this._footnotesUrl = url;
    return originalXHROpen.apply(this, arguments);
  };
  XMLHttpRequest.prototype.send = function (body) {
    const xhr = this;
    const url = xhr._footnotesUrl || "";
    if (url.includes(GRAPHQL_URL) || url.includes(TIMELINE_URL) || url.includes(DETAIL_URL) || url.includes(BOOKMARK_OPERATION)) {
      ...
      xhr.addEventListener("load", function () {
        try {
          ...
          const tweetsFound = extractAndCacheTweets(json);
          if (tweetsFound > 0) console.log(`[FootNotes] Cached ${tweetsFound} tweet(s) from XHR`);
        } catch (err) {}
      });
    }
  ...
  console.log("[FootNotes] Stable Fetch interceptor injected.");
})();