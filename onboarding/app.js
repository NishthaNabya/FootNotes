...
async function api(path, options = {}) {
  ...
  if (!response.ok) throw new Error(body.detail || "Footnote could not complete that action.");
  return body;
}
...
async function checkOllama() {
  ...
  try {
    const health = await api("/footnote-api/ollama-status");
    state.textContent = health.message;
    if (!health.runtime_available) {
      actions.classList.remove("hidden");
      $("ollamaGuidance").innerHTML = 'Install the free <a href="https://ollama.com/download/mac" target="_blank" rel="noreferrer">Ollama app for Mac</a>. Footnote will not install it for you. The two model downloads require about <b>2 GB</b> total.';
    } else if (health.missing_models?.length) {
      actions.classList.remove("hidden");
      $("ollamaGuidance").textContent = "Download the missing local models. Footnote will not start these downloads for you:";
...
function showStatus(data) {
  ...
  $("logPath").textContent = data.log_path || "~/Library/Logs/Footnote/footnote.log";
}

async function init() {
  state = await api("/footnote-api/setup");
...
$("chooseFolder").addEventListener("click", async () => {
  try { const result = await api("/footnote-api/choose-folder", { method:"POST", body:"{}" }); if (!result.cancelled) $("vaultPath").value = result.vault_path; } catch (error) { $("setupMessage").textContent = error.message; }
});
$("finish").addEventListener("click", async () => {
  ...
  try {
    const provider = document.querySelector("input[name=provider]:checked").value;
    await api("/footnote-api/setup", { method:"POST", body:JSON.stringify({ vault_path:$("vaultPath").value, provider }) });
    state = await api("/footnote-api/setup"); showStatus(state);
  } catch (error) { message.className="message error"; message.textContent=error.message; } finally { button.disabled=false; }
});
$("stop").addEventListener("click", async () => { $("stop").disabled=true; $("stop").textContent="Stopping…"; try { await api("/footnote-api/stop", {method:"POST",body:"{}"}); } catch {} });
