...
class OllamaTransport:
    ...
    def vector(text):
        ...
        if any(word in text for word in ("orbit", "homepage", "landing", "onboarding")):
...
class OllamaMemoryIntegrationTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_semantic_recall_note_related_and_resurfacing_use_ollama(self):
        ...
        noted = await self.save(
            ...
            "https://example.com/design", note="use this for Orbit landing page",
            tags=["reference"],
        )
        ...
        note_ranked = await server.hybrid_recall(server.load_vault_entries(), "Orbit homepage idea")
