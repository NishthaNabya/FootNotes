...
class FakeEmbeddingProvider:
    ...
    def _vector(text):
        text = text.lower()
        if any(word in text for word in ("orbit", "landing", "homepage", "onboarding")):
...
class RecallFoundationTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_note_can_arrive_while_capture_is_queued(self):
        ...
        pending = await self.update_note(entry_id, "use this for Orbit onboarding")
        ...
        self.assertEqual(entry["user_note"], "use this for Orbit onboarding")
    ...
    async def test_note_persists_and_participates_in_exact_recall(self):
        ...
        result = await self.update_note(entry["id"], "use this for Orbit landing page")
        ...
        self.assertEqual(reparsed["user_note"], "use this for Orbit landing page")
        self.assertEqual(reparsed["content"], "Generic visual design material.")

        ranked = await server.hybrid_recall(
            server.load_vault_entries(), "Orbit landing page"
        )
    ...
    async def test_user_note_participates_in_semantic_recall(self):
        ...
        await self.update_note(intended["id"], "use this for Orbit landing page")
        ranked = await server.hybrid_recall(
            server.load_vault_entries(), "thing I wanted for the Orbit homepage"
        )
        self.assertEqual(ranked[0]["id"], intended["id"])
    ...
    async def test_related_finds_semantic_neighbor_with_different_tags(self):
        ...
        # Cosine 0.70 is plausibly similar, but below Orbit's quality floor.
    ...
    async def test_related_excludes_obvious_duplicate(self):
        source = await self.save(
            ...
            "https://example.com/duplicate?utm_source=orbit",
    ...
    async def test_user_note_naturally_influences_relatedness(self):
        ...
        await self.update_note(source["id"], "use this for Orbit onboarding")
        await self.update_note(neighbor["id"], "idea for the Orbit homepage")
        related = server.related_memories(server.load_vault_entries(), source["id"])
